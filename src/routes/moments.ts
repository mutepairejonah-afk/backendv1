...
momentsRouter.post("/create-moment", requireAuth, (req, res) => rp(res, async () => {
  ...
  const fullPayload = { clerk_user_id: data.clerkUserId, text: data.text || null, image_url: data.imageUrl || null, video_url: data.videoUrl || null, audio_url: data.audioUrl || null, thumbnail_url: data.thumbnailUrl || null, mime_type: data.mimeType || null, duration_seconds: data.durationSeconds ?? null, expires_at: new Date(Date.now() + (data.expiresInHours ?? 24) * 3600000).toISOString() };
  let { data: moment, error } = await supabaseAdmin.from("moments").insert(fullPayload).select().single();
  // Some Render deployments may be running before the additive media migration.
  // Retry with the original text/image columns so text and image moments remain
  // usable while the migration is being applied.
  if (error && /column|schema cache|Could not find/i.test(error.message)) {
    const legacy = await supabaseAdmin.from("moments").insert({ clerk_user_id: data.clerkUserId, text: fullPayload.text, image_url: fullPayload.image_url }).select().single();
    moment = legacy.data;
    error = legacy.error;
  }
